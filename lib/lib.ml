...
let float_of_string_oe x =
  match Caml.float_of_string_opt x with
  | Some x -> Or_error.return x
  | None -> Or_error.error_string "Failed to parse float"
...
let parse_search_line line =
  ...
  | [ query; target; fident; bit_score; cigar ] ->
      let%bind fident = float_of_string_oe fident in
      let pident = fident *. 100.0 in
      let%bind bit_score = float_of_string_oe bit_score in
...
let parse_search_file fname =
  ...
  |> List.mapi ~f:(fun i line ->
         ...
         in
         (* Note that [Cigar.num_matches_exn] is the ungapped alignment length
            from the HFSP paper. *)
         let%map ungapped_aln_len =
           Cigar.num_matches search.cigar
           |> Or_error.tag
                ~tag:
                  [%string
                    "Error when calculating num_matches.  Probably int \
                     overflow."]
         in
         let score = hfsp_score search.pident ungapped_aln_len in
         {
           query = search.query;
